/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { FillColorWhereUniqueInput } from "./FillColorWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { FillColorUpdateInput } from "./FillColorUpdateInput";

@ArgsType()
class UpdateFillColorArgs {
  @ApiProperty({
    required: true,
    type: () => FillColorWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => FillColorWhereUniqueInput)
  @Field(() => FillColorWhereUniqueInput, { nullable: false })
  where!: FillColorWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => FillColorUpdateInput,
  })
  @ValidateNested()
  @Type(() => FillColorUpdateInput)
  @Field(() => FillColorUpdateInput, { nullable: false })
  data!: FillColorUpdateInput;
}

export { UpdateFillColorArgs as UpdateFillColorArgs };
